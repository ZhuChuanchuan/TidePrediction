...
//			+ "\\webapps\\DemoPredictTide\\files";
	...
	public boolean DoPredict(int predictInterval) { //predictInterval 预测多少天
		 ...
		 try {
			daySpan= CommonMethod.daysBetween("2013/1/1",endDate);
	...
	private Integer[] getRKLL(){
		...
		Integer[] RKLL=new Integer[6];
		try {
			Calendar c=Calendar.getInstance();
			c.setTime(dt);
			c.add(Calendar.DAY_OF_YEAR, -6);
			SimpleDateFormat sdf=new SimpleDateFormat("yyyy/MM/dd");
			String tempStartDate=sdf.format(c.getTime());
			 
			pre = conn.prepareStatement(sql);
			pre.setString(1,tempStartDate);
			pre.setString(2,endDate);
			result = pre.executeQuery();
			
			int x=0;
			System.out.println("大通流量：");
			while (result.next()) {
				...
				try{
					...
					System.out.println(Integer.parseInt(result.getString(2)));
				}catch(Exception e){
	...
	private void CreateTQ() {
        ...
        //System.out.println("查询时间参数："+calendar.getTime());
		...
		System.out.println(calendar.getTime()+" "+endCal.getTime()+" "+tempCal.getTime());
		*/
		int twoDayCursor=0;
		double rkll;
		while(calendar.compareTo(endCal)<=-1){
			...
			/*
			if(calendar.compareTo(tempCal)<=-1){ //一天半 取插值
			...
			//System.out.println(calendar.getTime()+" "+rkll);
		}
